...
app.post("/instalacion", async (req, res) => {
  try {
    ...
    const { Nombre, email, telefono, referencias, coordenadas, telefonocasa, domicilio, paquete, Hora, Minuto, Fecha, photo_data } = req.body.data;
 // Formatear la fecha y hora para enviar a la API de Mikrosystem

 
 const fecha_instalacion = `${Fecha} ${Hora}:${Minuto}`;
    const respuesta = await axios({
      ...
      data: {
        ...
        email: email,
        movil: telefono,
        Notas: `${referencias},\n${coordenadas},\n${paquete},${photo_data}`,// Concatenando
